...
export default defineConfig({
  ...
  // with `ENVHQ_DEFAULT_URL=http://localhost:3000 pnpm build`.
  // (Note: `pnpm dev` via tsx skips this define and falls back to localhost.)
  define: {
    __ENVHQ_DEFAULT_URL__: JSON.stringify(
      process.env.ENVHQ_DEFAULT_URL ?? "https://envhq.dev",
    ),
    ...
    __ENVHQ_VERSION__: JSON.stringify(pkg.version),
  },
});
