...
import { GET as listAccess, POST as upsertAccess } from "@/app/api/projects/[id]/access/route";
import { POST as createEnvironmentRoute } from "@/app/api/projects/[id]/environments/route";
...
describe("GET/POST /api/projects", () => {
  test("GET 200 lists accessible projects", async () => {
    ...
    const path = `/api/projects?orgId=${orgId}`;
...
describe("POST /api/projects/{id}/environments", () => {
  test("201 creates an environment, optionally cloning another's variables", async () => {
    ...
    const path = `/api/projects/${project.id}/environments`;
...
describe("GET/POST /api/projects/{id}/access, DELETE /api/projects/{id}/access/{grantId}", () => {
  test("GET 200 lists grants and environments", async () => {
    const { project, token } = await setup();
    const path = `/api/projects/${project.id}/access`;
...
describe("Project key management", () => {
  test("POST /api/projects/{id}/keys 201 self-registers, then 409 on a second registration", async () => {
    const { project, token } = await setup();
  ...
  test("POST /api/projects/{id}/keys 403 for a read-only token — the fixed isReadOnly gate", async () => {
  ...
  test("GET /api/projects/{id}/keys/me 404 ProjectKeyNotFound before registering, 200 after", async () => {
    const { project, token } = await setup();
    const path = `/api/projects/${project.id}/keys/me`;
    ...
    await call(registerProjectKey, `/api/projects/${project.id}/keys`, { id: project.id }, { method: "POST", token, json: { wrappedDek: "wrapped" } });
  ...
  test("GET /api/projects/{id}/keys/members and /pending reflect onboarding + wrap state", async () => {
    const { project, userId, token } = await setup();
    await createUserKeys(userId);
    const pendingPath = `/api/projects/${project.id}/keys/pending`;
    ...
    const membersPath = `/api/projects/${project.id}/keys/members`;
  ...
  test("GET /api/projects/{id}/keys/rotation-status 200", async () => {
    const { project, token } = await setup();
    const path = `/api/projects/${project.id}/keys/rotation-status`;
  ...
  test("POST /api/projects/{id}/keys/rotate 200 migrates a batch, 403 for a read-only token", async () => {
    ...
    const path = `/api/projects/${project.id}/keys/rotate`;
  ...
  test("POST /api/projects/{id}/keys/rotate/finalize 403 for a read-only token, 200 once nothing is pending", async () => {
    const { project, token, readOnlyToken } = await setup();
    const path = `/api/projects/${project.id}/keys/rotate/finalize`;
