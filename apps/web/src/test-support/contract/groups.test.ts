...
import { DELETE as removeMember } from "@/app/api/v1/groups/[id]/members/[userId]/route";
import { GET as listMembers, POST as addMember } from "@/app/api/v1/groups/[id]/members/route";
...
describe("GET/POST /api/v1/groups", () => {
  test("GET 200 lists an org's groups", async () => {
    ...
    const path = `/api/v1/groups?orgId=${orgId}`;
...
describe("GET/POST /api/v1/groups/{id}/members, DELETE /api/v1/groups/{id}/members/{userId}", () => {
  test("GET 200 lists members", async () => {
    ...
    const path = `/api/v1/groups/${group.id}/members`;
  ...
  test("DELETE 200 removes a member", async () => {
    const { group, memberUserId, adminToken } = await setup();
    await seedGroupMember(group.id, memberUserId);
    const path = `/api/v1/groups/${group.id}/members/${memberUserId}`;
  ...
  test("DELETE 404 for a userId that was never a member", async () => {
    ...
    const path = `/api/v1/groups/${group.id}/members/${crypto.randomUUID()}`;
...
describe("DELETE /api/v1/groups/{id}", () => {
  test("200 deletes the group", async () => {
    const { group, adminToken } = await setup();
    const path = `/api/v1/groups/${group.id}`;
